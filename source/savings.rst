...
People earn income, for the large part from work, but also from pensions and other sources. From that income, they spend on goods which they consume, some of which they leave for their heirs. There is no need for a one-to-one relationship between when we earn income and when we spend it. Luckily, we can save some income for later when we earn a lot  and we can borrow when income is low. There is a lot of saving, and then dissaving over the life-cycle. For example, paying mandatory pension contributions reduces potential expenditures today against a promise to receive income in the future that allows to spend on goods when old. Paying taxes has also a savvings component since lots of government spending is devoted to the young (who earn no income) and the old, who for example consume a lot of health care. In this set of lectures, we will be mostly concerned with voluntary private savings. It is useful to think of savings as a stock, with contributions, returns and withdrawals being the flows. Let the stock of all savings, say at market value, be :math:`w_t`. If we denote by :math:`s_t` the contributions (when positive) and withdrawals (when negatives), :math:`r_t` the effective rate of return, then the stock evolves according to 

.. math::
        w_{t+1} = (1+r_t) w_{t} + s_{t}

Turns out we have limited information on stocks but also on flows. Administrative data in most countries, with the exception of some scandinavian countries, is not available on savings stock and flows such that we could follow the equation above. Hence, we must resort to surveys to get most of our information on savings flows and stocks. 
...
.. math::
   c^*_2 = \frac{\eta}{1 + R^{-1} \eta + R^{-2}\eta} PV_{y} \\
   c^*_3 = \frac{\eta^2}{1 + R^{-1} \eta + R^{-2}\eta} PV_{y} 
